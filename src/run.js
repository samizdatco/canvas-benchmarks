...
import {tests, libs, initialize, sysInfo, OUTPUT_DIR} from './config.js'
...
async function testLibrary(libID, testID){
  ...
  saveSnapshot(`${OUTPUT_DIR}/snapshots/${testID}_${libID}.${ext}`, await test(lib))

  console.log(toJSON({ test:testID, lib:lib.name, rounds, ms }))
}

function saveSnapshot(filename, snapshot){
...
async function main(){
  ...
  if (cmd=="test" && testID=="cold-start") testLibraryInit(libID)
    ...
    let benchmarks = await runTests(Object.keys(tests)),
        ...
        [md, svg] = formatResults(results)

    mkdir(OUTPUT_DIR)
    writeFileSync(`${OUTPUT_DIR}/data.json`, toJSON(results))
    writeFileSync(`${OUTPUT_DIR}/index.md`, md)
    writeFileSync(`${OUTPUT_DIR}/bars.svg`, svg)
